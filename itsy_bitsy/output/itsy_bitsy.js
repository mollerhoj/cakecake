...
(function() {
  ...
  Entity = (function() {
    ...
    Entity.prototype.start_x = 0;

    Entity.prototype.start_y = 0;

    Entity.prototype.start_z = 0;

    Entity.prototype.start_width = void 0;

    Entity.prototype.start_height = void 0;

    Entity.prototype.start_radius = void 0;

    Entity.prototype.start_angle = 0;
    ...
    function Entity() {
      this.hit_hash = new Object();
      this.unique = Math.generate_unique();
    }
    ...
    Entity.prototype.update_position = function() {
      if (this.physics && this.body) {
        return this.body.SetPosition(new b2Vec2(this.x / AppData.pixel_per_meter, this.y / AppData.pixel_per_meter));
      }
    };
    ...
    Entity.prototype.set_angle = function(angle) {
      this.angle = angle;
      if (this.physics && this.body) {
        return this.body.SetAngle((-this.angle / 360) * Math.PI * 2);
      }
    };

    Entity.prototype.set_width = function(width) {
      return this.width = width;
    };

    Entity.prototype.set_height = function(height) {
    ...
    Entity.prototype.step = function() {
      ...
      if (this.physics && this.body) {
        ...
        this.x = p.x * AppData.pixel_per_meter;
        this.y = p.y * AppData.pixel_per_meter;
        this.sprite.angle = -this.body.GetAngle() / (Math.PI * 2) * 360;
      }
      return null;
    };
    ...
    Entity.prototype.move_towards = function(x, y, speed) {
      var dir;
      dir = this.angle_to(x, y);
      set_x(this.x + Math.cos(dir / 180 * Math.PI) * speed);
      return set_y(this.y + Math.sin(dir / 180 * Math.PI) * speed);
    };

    Entity.prototype.angle_to = function(x, y) {
    ...
    Entity.prototype.reset = function() {
      ...
      return this.angle = this.start_angle;
    };
  ...
  EntityFactory = (function() {
    ...
    EntityFactory.prototype.physics = null;

    EntityFactory.prototype.world = null;

    function EntityFactory(world, physics) {
      this.world = world;
      this.physics = physics;
    }

    EntityFactory.prototype.spawn = function(name, x, y) {
      var cl, entity;
      if (x == null) {
        x = 0;
      }
      ...
      if (entity.width === void 0) {
        entity.width = entity.sprite.width;
      }
      if (entity.height === void 0) {
        entity.height = entity.sprite.height;
      }
      if (entity.radius === void 0) {
        entity.radius = (entity.width + entity.height) / 4;
      }
      entity.art = this.world.art;
      if (AppData.physics) {
        if (entity.physics) {
          entity.body = this.physics.build_dynamic(x, y, entity.width, entity.height, entity.radius, entity.physics);
  ...
  Physics = (function() {
    ...
    Physics.prototype.edges = null;

    Physics.prototype.visible = false;

    Physics.prototype.time_step = 120;

    Physics.prototype.mouse_hitbox = null;

    function Physics() {
      this.create_world();
      this.setup_debug_draw();
      this.setup_mouse_hitbox();
      this.setup_contact_listener();
    }

    Physics.prototype.setup_mouse_hitbox = function() {
      var fd, mouse_hitbox_def;
      mouse_hitbox_def = new b2BodyDef();
      mouse_hitbox_def.type = b2Body.b2_dynamicBody;
      this.mouse_hitbox = this.world.CreateBody(mouse_hitbox_def);
      this.mouse_hitbox.SetUserData('MOUSE');
      fd = new b2FixtureDef;
      fd.shape = new b2PolygonShape;
      fd.shape.SetAsBox(1 / AppData.pixel_per_meter, 1 / AppData.pixel_per_meter);
      fd.isSensor = true;
      return this.mouse_hitbox.CreateFixture(fd);
    };

    Physics.prototype.build_edges = function() {
      var edges_def, fix_def;
      edges_def = new b2BodyDef();
      this.edges = this.world.CreateBody(edges_def);
      this.edges.SetUserData('EDGE');
      fix_def = new b2FixtureDef;
      this.build_edge(0, 0, AppData.width, 0, fix_def);
      this.build_edge(0, AppData.height, AppData.width, AppData.height, fix_def);
      this.build_edge(0, 0, 0, AppData.height, fix_def);
      return this.build_edge(AppData.width, 0, AppData.width, AppData.height, fix_def);
    };

    Physics.prototype.build_dynamic = function(x, y, width, height, radius, physics) {
    ...
    Physics.prototype.step = function() {
      this.mouse_hitbox_set_position();
      return this.world.Step(1 / this.time_step, 3, 2);
    };

    Physics.prototype.mouse_hitbox_set_position = function() {
      var x, y;
      x = Math.max(0, Math.min(AppData.width, Keyboard.MOUSE_X));
      y = Math.max(0, Math.min(AppData.height, Keyboard.MOUSE_Y));
      return this.mouse_hitbox.SetPosition(new b2Vec2(x / AppData.pixel_per_meter, y / AppData.pixel_per_meter));
    };
    ...
    Physics.prototype.setup_contact_listener = function() {
      ...
      contact_listener.BeginContact = function(contact) {
        ...
        if (a instanceof Entity && b instanceof Entity) {
          ...
          return b.hit_hash[a.unique] = a;
        } else if (a === 'MOUSE' && b instanceof Entity) {
          return b.hit_hash['MOUSE'] = true;
        } else if (b === 'MOUSE' && a instanceof Entity) {
          return a.hit_hash['MOUSE'] = true;
        }
      };
      contact_listener.EndContact = function(contact) {
        ...
        if (a instanceof Entity && b instanceof Entity) {
          ...
          return delete b.hit_hash[a.unique];
        } else if (a === 'MOUSE' && b instanceof Entity) {
          return delete b.hit_hash['MOUSE'];
        } else if (b === 'MOUSE' && a instanceof Entity) {
          return delete a.hit_hash['MOUSE'];
        }
      };
    ...
    Physics.prototype.create_world = function() {
      return this.world = new b2World(new b2Vec2(0, 0), true);
    };
    ...
    Physics.prototype.build_edge = function(x1, y1, x2, y2, fix_def) {
      ...
      return this.edges.CreateFixture(fix_def);
    };
  ...
  Sprite = (function() {
    ...
    Sprite.prototype.width = void 0;

    Sprite.prototype.height = void 0;
    ...
    function Sprite(name, x, y) {
      ...
      this.x = x;
      this.y = y;
      this.width = Game.images[name].width;
      this.height = Game.images[name].height;
    }

    Sprite.prototype.draw = function() {
      var height, image, width, x, y;
      image = this._get_image();
      x = this.x - this.width / 2;
      y = this.y - this.height / 2;
      if (this.angle === 0 && this.w === image.width && this.h === image.height) {
        return Game.context.drawImage(image, 0, 0, Math.round(this.width), Math.round(this.height), Math.round(x), Math.round(y), Math.round(this.width), Math.round(this.height));
      } else {
        width = image.width;
        height = image.height;
        Game.context.save();
        Game.context.translate(x + this.width / 2, y + this.height / 2);
        Game.context.rotate(-Math.PI / 180 * (this.angle % 360));
        Game.context.scale(this.width / width, this.height / height);
        Game.context.translate(-this.width / 2, -this.height / 2);
        Game.context.drawImage(image, 0, 0, this.width, this.height);
        return Game.context.restore();
      }
    };
  ...
  Text = (function() {
    ...
    Text.prototype.draw = function(string, x, y) {
      ...
      if (this.angle !== 0) {
        ...
        Game.context.rotate(Math.PI / 180 * this.angle);
  ...
  Math.generate_unique = function(length) {
    var id;
    if (length == null) {
      length = 8;
    }
  ...
  World = (function() {
    ...
    function World() {
      ...
      this.factory = new EntityFactory(this, this.physics);
    }
    ...
    World.prototype.load_level = function(name, index) {
      ...
      for (number in _ref) {
        ...
        if (dataobject.height) {
          _results.push(s.set_height(dataobject.height));
        } else {
    ...
    World.prototype.spawn = function(name, x, y) {
      var entity;
      entity = this.factory.spawn(name, x, y);
    ...
    World.prototype.nearest = function(c, self) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        e = _ref[_i];
        if (e.name === c && e !== self) {
          distance = this.entities_distance(self, e);
          if (shortest === void 0 || distance < shortest) {
            nearest = e;
            shortest = distance;
          }
        }
    ...
    World.prototype.entities_distance = function(e1, e2) {
  ...
  Bullet = (function(_super) {
    ...
    Bullet.prototype.step = function() {
      ...
      return this.y -= Math.sin(this.angle / 180 * Math.PI) * this.speed;
    };
  ...
  Button = (function(_super) {
    ...
    Button.prototype.physics = {
      shape: 'rectangle'
    };

    Button.prototype.body = null;
    ...
    Button.prototype.init = function() {
      return this.world.physics.build_edges();
    };
      ...
      if (this.touch('MOUSE')) {
        if (this.menu) {
          if (Keyboard.press('MOUSE_LEFT')) {
            this.menu.press(this.id);
          }
          if (Keyboard.mouse_moves) {
            return this.menu.set_index(this.id);
          }
        }
      }
    };
  ...
  Fly = (function(_super) {
    ...
    Fly.prototype.draw = function() {
      this.x = this.start_x + Math.cos(this.ani / 180 * Math.PI) * 5;
      this.y = this.start_y + Math.sin(this.ani / 180 * Math.PI) * 5;
  ...
  Menu_1D = (function(_super) {
    ...
    Menu_1D.prototype.init = function() {
      ...
      for (i = _i = 0, _ref = this.button_n; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        ...
        b.id = i;
        b.set_x(60);
        b.set_y(this.y + i * this.button_space);
        b.sprite = null;
  ...
  Menu_2D = (function(_super) {
    ...
    Menu_2D.prototype.init = function() {
      ...
      b = this.world.spawn('Button');
      b.set_x(this.x - this.button_x_space);
      b.set_y(this.y);
      b.sprite = null;
      ...
      for (i = _j = 1, _ref1 = this.button_n; 1 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 1 <= _ref1 ? ++_j : --_j) {
        ...
        if (b) {
          ...
          b.text.font = 'Norwester';
          b.text.font_size = 23;
          b.set_y(this.y + Math.floor((i - 1) / this.cols) * this.button_y_space);
          b.set_x(this.x + ((i - 1) % this.cols) * this.button_x_space);
  ...
  Spider = (function(_super) {
    ...
    Spider.prototype.radius = 5;

    Spider.prototype.speed = 3;
    ...
    Spider.prototype.webgrap_body = null;

    Spider.prototype.init = function() {
      var webgrap_bd;
      this.body.ApplyForce(new b2Vec2(0, this.gravity), this.body.GetWorldCenter());
      webgrap_bd = new b2BodyDef();
      this.webgrap_body = this.world.physics.world.CreateBody(webgrap_bd);
      this.joint_last_angle = this.angle + 90;
      this.rope = [];
    ...
    Spider.prototype.dis_joint = function(p) {
      ...
      jointDef.Initialize(this.webgrap_body, this.body, p, this.body.GetWorldCenter());
    ...
    Spider.prototype.shoot_bullet = function() {
      if (!this.bullet && !this.joint) {
        ...
        return this.bullet.angle = this.aim2;
      }
    };
    ...
    Spider.prototype.draw = function() {
      ...
      if (this.joint) {
        vec = this.joint.GetAnchorB().Copy();
        vec.Subtract(this.joint.GetAnchorA());
        this.angle = 90 - vec.angle() / (Math.PI * 2) * 360;
      } else {
        this.angle = this.aim2 - 90;
      }
  ...
  Builder = (function() {
    ...
    Builder.prototype.step = function() {
      ...
      if (this.hold) {
        ...
        if (Keyboard.hold('Q')) {
          this.hold.set_angle(this.hold.angle + 1);
        }
        if (Keyboard.hold('E')) {
          this.hold.set_angle(this.hold.angle - 1);
        }
  ...
  Editor = (function() {
    ...
    Editor.prototype.level_change = function() {
      ...
      this.world.destroy_all();
      this.world.load_level(this.level);
      return this.level_selector.blur();
    };

    Editor.prototype.save_type_change = function() {
      this.builder.save_type = this.save_type_selector.value;
      return this.save_type_selector.blur();
    };

    Editor.prototype.debug_draw_change = function() {
      this.world.physics.visible = this.debug_draw_checkbox.checked;
      return this.debug_draw_checkbox.blur();
    };
    ...
    Editor.prototype.entity_change = function() {
      this.builder.entity = this.entity_selector.value;
      return this.entity_selector.blur();
    };
    ...
    Editor.prototype.grid_move = function() {
      ...
      this.grid.height = parseInt(this.grid_height.value);
      this.grid_x.blur();
      this.grid_y.blur();
      this.grid_width.blur();
      return this.grid_height.blur();
    };
    ...
    Editor.prototype.toggle_grid = function() {
      ...
      return this.grid_toggle.blur();
    };

    Editor.prototype.toggle_pause = function() {
      this.set_pause(!Game.pause);
      return this.pause_button.blur();
    };
  ...
  Grid = (function() {
    ...
    function Grid() {
      ...
      this.width = AppData.pixel_per_meter;
      this.height = AppData.pixel_per_meter;
    }
